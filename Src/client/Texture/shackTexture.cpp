...
zap::shackTexture::shackTexture(zap::Gui &base) : _base(base)
{
}

void zap::shackTexture::initTexture()
{
	initBck();
	initHeader();
	initNbPlayer();
	...
	_base._monitor->addFuncLoop(-1, &zap::shackTexture::updateShack, this);
}

void zap::shackTexture::updateShack()
{
...
void zap::shackTexture::initClose()
{
...
void zap::shackTexture::initArrow()
{
...
void zap::shackTexture::initData()
{
	auto container = new zap::Container(sf::IntRect(10, 380, WIDTH - 10, HEIGHT - 380));
	container->setBoolUsed(false);

	auto title = new zap::Text("extra/Gobold.otf", sf::IntRect(0, 0, 100, 20), "Tile resources:");
...
void zap::shackTexture::initNbPlayer()
{
	auto container = new zap::Container(sf::IntRect(10, 260, WIDTH - 10, 100));
	container->setBoolUsed(false);

	auto title = new zap::Text("extra/Gobold.otf", sf::IntRect(0, 0, 100, 20), "Tile Data:");
	title->setColor(sf::Color::Black);
	container->addObjectList("title", title);

	_nb_player = new zap::Text("extra/arial.ttf", sf::IntRect(0, 25, 100, 20), "??? player(s)");
...
void zap::shackTexture::initHeader()
{
	auto container = new zap::Container(sf::IntRect(0, 0, WIDTH, 235));
	container->setBoolUsed(false);
	std::string text = "0 : 0";

	auto user = new zap::Sprite("extra/gui/floor_icon.png", sf::IntRect(70, 20, 160, 160));
	_nickShack = new zap::Text("extra/arial.ttf", sf::IntRect(145 - (int)(text.size() / 2 * 13), 180, (int)(text.size() / 2 * 13), 26), text);
