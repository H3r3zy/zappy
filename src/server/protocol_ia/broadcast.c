...
void broadcast_cmd(server_t *server, client_t *client, char *arg)
{
	...
	for (client_t *clt = server->clients; clt; clt = clt->next) {
		if (clt != client) {
			dx = MAP_SHORTEST_PATH((int)clt->entity->pos.x -
			(int)client->entity->pos.x, (int)server->map.size.x);
			dy = MAP_SHORTEST_PATH((int)clt->entity->pos.y -
			(int)client->entity->pos.y, (int)server->map.size.y);
