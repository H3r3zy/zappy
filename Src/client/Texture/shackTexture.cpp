...
void irc::shackTexture::updateShack()
{
	...
	for (auto &&it : list_msg) {
		if (it.getCommandName() == "bct" && _base._comm._shack._pos.first == it.getCommand()[0] && _base._comm._shack._pos.second == it.getCommand()[1]) {
			_base._comm._shack.ressources.q0 = it.getCommand()[2];
			_base._comm._shack.ressources.q1 = it.getCommand()[3];
			_base._comm._shack.ressources.q2 = it.getCommand()[4];
			_base._comm._shack.ressources.q3 = it.getCommand()[5];
			_base._comm._shack.ressources.q4 = it.getCommand()[6];
			_base._comm._shack.ressources.q5 = it.getCommand()[7];
			_base._comm._shack.ressources.q6 = it.getCommand()[8];
			break;
		}
	}
